...
## index(インデックス)
* インデックス作成にはテーブルレベルのロックが走る
* インデックスの定義で使用される全ての関数と演算子は、immutable(不変)でなければなりません。 つまり、結果は入力引数にのみに依存し、(他のテーブルの内容や現時刻などの)外部からの影響を受けてはならない。※公式マニュアル参照

* concurrentlyオプションによるインデックス作成
  * 対象テーブルに対する同時挿入、更新、削除を防止するようなロックを獲得せずにインデックスを作成する。
  * テーブルを2回スキャンする。
    * 1.インデックスの存在をカタログに登録
    * 2.スナップショットを取得し、インデックスを作成。ShareLockと競合するトランザクションが全て終了するまで待つ。
    * 3.古いスナップショットを参照しているトランザクションが終了するのを待ち、2の最中に発生した差分を反映
  * 潜在的にそのインデックスを更新または使用する可能性がある、実行中のすべてのトランザクションが終わるまで待機する。
  * 通常の方式よりも総作業時間がかかり、また、完了するまでの時間が非常に長くなる。
